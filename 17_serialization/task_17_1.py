...
def parse_sh_version(sh_ver_string):
    regex = ('Cisco IOS .+ Version (?P<ios>\S+),'
            '|file is "(?P<image>\S+)"'
            '|uptime is (?P<uptime>.+)')
    match_iter = re.finditer(regex, sh_ver_string)
    ...
    with open(csv_filename, 'w+') as dst:
        ...
        for file in data_filenames:
            ...
            with open(file, 'r') as src:
                result = (host, *parse_sh_version(src.read()))
                writer.writerow(result)


if __name__ == '__main__':
    write_inventory_to_csv(sh_version_files, "task")
